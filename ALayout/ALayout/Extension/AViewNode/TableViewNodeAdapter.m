...
const void* _TableViewNodeAdapterKeyForTableView = &_TableViewNodeAdapterKeyForTableView;

+ (instancetype)tableViewNodeAdapter:(UITableView*)tableView
{
    if(!tableView)
    {
        return nil;
    ...
    TableViewNodeAdapter* viewNodeAdapter = objc_getAssociatedObject(tableView, _TableViewNodeAdapterKeyForTableView);
    if(!viewNodeAdapter)
    {
        viewNodeAdapter = [[TableViewNodeAdapter alloc] initWithTableView:tableView];
        objc_setAssociatedObject(tableView, _TableViewNodeAdapterKeyForTableView, viewNodeAdapter, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return viewNodeAdapter;
}
