...
- (CGSize)calcChild:(MeasureSpec)widthSpec heightSpec:(MeasureSpec)heightSpec size:(CGSize)size
{
    ...
    for(UIView* child in subviews)
    {
        ...
        MeasureSpec useHeightSpec = heightSpec;
        if(layoutParams.layout_height == LayoutParams_MATCH_PARENT)
        {
            useHeightSpec.mode = MeasureSpec_EXACTLY;
            useHeightSpec.size = size.height;
        }
        [self measureChildWithMargins:child parentWidthSpec:widthSpec widthUsed:0 parentHeightSpec:useHeightSpec heightUsed:0];
