...
- (void)parseAttr:(AttributeReader*)attrReader useDefault:(BOOL)useDefault
{
    ...
    if ([attrReader hasKey:ValueKey(A_ios_action)])
    {
        ...
        BOOL attached = [(UIControl*)self attachAction:action withTarget:attrReader.target forControlEvents:self.defaultControlEvent];
        if(!attached)
        {
            [(UIControl*)self detachActionWithTarget:attrReader.target forControlEvents:self.defaultControlEvent];
        }
    }
...
- (UIControlEvents)defaultControlEvent
{
